  ...
  const handleClickSignIn = () => {
    if (!isFetchingToken) {
      ...
      fetch('https://api.themoviedb.org/3/authentication/token/new', GET)
        ...
        .then(token => {
          window.location.replace(`https://www.themoviedb.org/authenticate/${token.request_token}?redirect_to=https://cineverse-bigbro.vercel.app/`);
  ...
  return (
    <span
      ...
      {sessionId === '' ? (
        ...
        <div className='relative flex flex-col gap-0 px-3 w-full flex-shrink-0'>
          ...
          <button
            ...
            />
            <p style={navBtn === 'favorite' ? { color: 'white' } : { color: '#A8A29E' }}>Favorited</p>
          </button>
          <button
            className='font-bold text-start py-2 px-3 rounded-md flex flex-row justify-start gap-3 items-center'
            ...
            <Star
              ...
              className='h-4 w-auto'
            />
            <p style={navBtn === 'rated' ? { color: 'white' } : { color: '#A8A29E' }}>Rated</p>
          </button>
        </div>
      )}
      <div className='border-b border-stone-600 relative w-full h-6 flex-shrink-0'></div>
      ...
      <div className='relative w-full h-auto px-3 text-stone-600 flex flex-col text-sm flex-shrink-0'>
        <a
          className='font-bold cursor-pointer hover:text-stone-300'
        ...
        <a
          className='font-bold cursor-pointer hover:text-stone-300'
          href='https://github.com/AnhBigBrother/cineverse'
          target='_blank'>
          Source code
        </a>
        <a
        ...
        <br />
        <p>
          Lorem ipsum dolor sit amet, consectetur adipiscing elit. Praesent placerat nulla at gravida vulputate. In elementum et mauris cursus sollicitudin. Maecenas eu dictum sem. Aenean blandit purus et diam auctor sollicitudin. Maecenas
          tempor, arcu non scelerisque pharetra, urna risus porta purus, ut scelerisque velit metus non ante.
        </p>
      </div>
    </span>
  );
