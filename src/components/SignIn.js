...
const SignIn = () => {
			...
			fetch("https://api.themoviedb.org/3/authentication/token/new", GET)
				...
				.then((token) => {
					window.location.replace(
						`https://www.themoviedb.org/authenticate/${token.request_token}?redirect_to=${APP_URL}`
					)
	...
	const handleClickOutSideUser = (e) => {
		if (
			userNInf.current &&
			inf.current.classList.contains("flex") &&
			!userNInf.current.contains(e.target)
		) {
			inf.current.classList.toggle("hidden")
			inf.current.classList.toggle("flex")
		}
	...
	return (
		<>
			{sessionId === "" ? (
				<button
					className='relative h-10 w-auto px-3 flex flex-row gap-2 items-center rounded-full border border-amber-300 hover:bg-stone-700 cursor-pointer'
				...
				<div
					...
					title='Your profile'>
					<button
						className='h-10 w-10 rounded-full text-center font-bold flex justify-center items-center bg-amber-800'
						onClick={handleClickUser}>
						<p>{userInf.name?.charAt(0) || userInf.username?.charAt(0)}</p>
					</button>
					<div
						ref={inf}
						className='absolute top-12 right-1 w-auto h-auto py-3 bg-neutral-800 rounded-lg hidden flex-col gap-3 min-w-40'>
						...
						<div className='w-auto flex flex-col gap-0 px-3'>
							<button
								className='font-bold text-start py-1 px-3 rounded-md hover:bg-neutral-500 flex flex-row justify-start gap-3 items-center'
								onClick={() => handleClickInfBtn("favorite")}>
								<Heart className='fill-white h-4 w-auto' />
								<p>Favorite</p>
							</button>
							<button
								className='font-bold text-start py-1 px-3 rounded-md hover:bg-neutral-500 flex flex-row justify-start gap-3 items-center'
								onClick={() => handleClickInfBtn("rated")}>
								<Star className='fill-white h-4 w-auto' />
								<p>Rated</p>
							</button>
						</div>
						<div className='w-full px-5'>
						...
						<div className='w-auto px-3'>
							<button
								className='font-bold w-full text-start py-1 px-3 rounded-md hover:bg-neutral-500 flex flex-row justify-start gap-3 items-center'
								onClick={handleClickSignOut}>
								<SignOut className='fill-white h-4 w-auto' />
								<p>Sign out</p>
							</button>
						</div>
					</div>
