...
const RateIt = ({ mediaType, id }) => {
    ...
    const handleClickRateButton = () => {
        if (sessionId !== '') {
            toolTip.current.textContent = 'Your ratting have been saved!';
            ...
            toolTip.current.textContent = 'You have to sign in!';
    ...
    const handleRate = (currentRating) => {
        ...
        fetch(`https://api.themoviedb.org/3/${mediaType}/${id}/rating?session_id=${sessionId}`, POST)
    ...
    return (
        <div ref={rateComponent} className="relative w-auto h-auto flex flex-col justify-center items-start">
            <button className="opacity-70 hover:opacity-100 hover:scale-110 duration-500" title="Rate it!"><Star className='fill-white h-8 w-8' onClick={handleClickRateButton} /></button>
            ...
            <p ref={toolTip} className='absolute text-nowrap -right-16 xl:right-auto xl:-left-4 top-10 z-10 p-2 bg-emerald-800 rounded-md' style={{ display: 'none' }}>Your ratting have been saved!</p>
        </div>
    )
}
