...
const People = () => {
    ...
    const [data, loading, setData] = useFetch(`https://api.themoviedb.org/3/person/popular?language=en-US&page=1`);

    const fetchMore = () => {
        let page = data.page + 1;
        if (page < data.total_pages) {
            fetch(`https://api.themoviedb.org/3/person/popular?language=en-US&page=${page}`, GET)
                .then(res => res.json())
                .then(newData => {
    ...
    return (
        <div className="relative w-full h-auto flex flex-col gap-3 p-5 justify-start items-center">
            <p className="text-2xl md:text-3xl font-bold">Popular people</p>
            {loading
                ? <PeopleSkeleton />
                : <InfiniteScroll
