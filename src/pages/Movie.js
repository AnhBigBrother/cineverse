...
const Movie = () => {
    ...
    const [data, loading, setData] = useFetch(`https://api.themoviedb.org/3/discover/movie?include_adult=true&language=en-US&page=1&sort_by=${sorttype}${genres}`);


    const fetchMore = () => {
        let page = data.page + 1;
        if (page < data.total_pages) {
            fetch(`https://api.themoviedb.org/3/discover/movie?include_adult=true&language=en-US&page=${page}&sort_by=${sorttype}${genres}`, GET)
                .then(res => res.json())
                .then(newData => {
    ...
    return (
        <div className="relative w-full h-full p-6 flex flex-col gap-6 justify-center items-center">
            <MovieAndTvSelector mediaType='movie' setGenres={setGenres} setSorttype={setSorttype} />
            {loading
                ? <MovieSkeleton />
