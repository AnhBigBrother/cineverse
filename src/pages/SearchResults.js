...
const SearchResults = () => {
    ...
    const [data, loading, setData] = useFetch(`https://api.themoviedb.org/3/search/${filter}?query=${query}&include_adult=true&language=en-US&page=1`);

    const fetchMore = () => {
        let page = data.page + 1;
        if (page < data.total_pages){
            fetch(`https://api.themoviedb.org/3/search/${filter}?query=${query}&include_adult=true&language=en-US&page=${page}`, GET)
            .then(res => res.json())
            .then(newData => {
    ...
    return (
        <div className="relative w-full flex flex-col items-center gap-5 p-5 md:p-10">
            <div className="flex flex-row gap-5">
                <p className="text-2xl md:text-3xl font-bold">Result:</p>
                <Select
                    name="filter"
                    defaultValue={'multi'}
                    options={option}
