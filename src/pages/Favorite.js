            ...
            {display && <div className='w-full h-40 md:h-56 rounded-lg grid-container-2 border border-slate-900 shadow-lg shadow-black'>
                ...
                <div className='w-full flex flex-col gap-[2px] md:gap-2 px-2 py-2 sm:px-5'>
                    ...
                    <div className='w-auto h-7 flex flex-row gap-2 justify-start items-center'>
                        ...
                        <p className='font-bold text-nowrap text-base md:text-lg opacity-75'>Rate it!</p>
                        <div className='w-1 sm:w-6 lg:w-12'></div>
                        <button className='w-auto h-auto' onClick={handleClickMinus} title='Remove favorite'><Minus className='w-7 h-7 cursor-pointer fill-white opacity-75 hover:opacity-100 hover:scale-110 duration-300'/></button>
...
const Favorite = () => {
    ...
    const [data, loading, setData] = useFetch(`https://api.themoviedb.org/3/account/account_id/favorite/${mediaType}?language=en-US&page=1&session_id=${sessionId}&sort_by=created_at.desc`)
    ...
    const fetchMore = () => {
        ...
        if (page < data.total_pages) {
            fetch(`https://api.themoviedb.org/3/account/account_id/favorite/${mediaType}?language=en-US&page=${page}&session_id=${sessionId}&sort_by=created_at.desc`, GET)
                .then(res => res.json())
                .then(newData => {
    ...
    return (
        <div className='relative w-full flex flex-col gap-5 py-6 sm:py-10'>
            <div className='w-full px-6 sm:px-10 flex flex-row justify-start items-center gap-3 md:gap-5 flex-wrap'>
                <p className='text-2xl font-bold'>Your favorites:</p>
